...
static const char *codes[][2] = {
  ...
  {"444", "444 - Connection closed without sending any headers"},
...
substring (const char *str, int begin, int len)
{
  ...
  if ((buffer = (char*)xmalloc (len + 1)) == NULL)
    return NULL;
...
char *
convert_date (char *result, char* zone, char *data, const char *from, const char *to,
              int size)
{
  struct tm *now_tm;
  struct tm tm;
  ...
  if (tm.tm_zone != NULL)
	  snprintf(zone, ZONE_LEN, "%s", tm.tm_zone);
...
int_to_str (int d)
{
  char *s = (char*)xmalloc (snprintf (NULL, 0, "%d", d) + 1);
  sprintf (s, "%d", d);
...
{
  char *s = (char*)xmalloc (snprintf (NULL, 0, "%.2f", d) + 1);
  sprintf (s, "%.2f", d);
...
left_pad_str (const char *s, int indent)
{
  ...
  buf = (char*)xmalloc (snprintf (NULL, 0, "%*s", indent, s) + 1);
...
escape_str (const char *src)
{
  ...
  p = (unsigned char *) src;
  q = dest = (char*)xmalloc (strlen (src) * 4 + 1);
